...
static bool wire_send_frame(int fd, const void *payload, size_t len)
{
    if (len > MAX_WIRE_MESSAGE) return false;
    uint32_t network_len = htonl((uint32_t)len);
    return write_all(fd, &network_len, sizeof(network_len))
        && write_all(fd, payload, len);
}

static bool wire_receive_frame(int fd, uint8_t **payload, size_t *len)
{
    uint32_t network_len;
    if (!read_all(fd, &network_len, sizeof(network_len))) return false;
...
static bool handle_scrollbar(GhosttyTerminal terminal,
                             ...
                             bool *dragging, bool *terminal_changed)
{
...
static bool input_send(int fd, InputWire *outgoing)
{
    msgpack_sbuffer packed;
    msgpack_sbuffer_init(&packed);
    bool ok = input_pack(outgoing, &packed)
        && wire_send_frame(fd, packed.data, packed.size);
    outgoing->len = 0;
    msgpack_sbuffer_destroy(&packed);
    return ok;
}

static bool input_receive(int fd, InputWire *incoming)
{
    uint8_t *packed = NULL;
    size_t packed_len = 0;
    incoming->len = 0;
    bool ok = wire_receive_frame(fd, &packed, &packed_len)
        && input_unpack(packed, packed_len, incoming);
    free(packed);
    return ok;
}

static bool snapshot_send(int fd, const RenderSnapshotWire *outgoing)
{
    msgpack_sbuffer packed;
    msgpack_sbuffer_init(&packed);
...
static bool snapshot_receive(int fd, RenderSnapshotWire *incoming)
{
    uint8_t *packed = NULL;
    size_t packed_len = 0;
    incoming->len = 0;
    bool ok = wire_receive_frame(fd, &packed, &packed_len)
        && snapshot_unpack(packed, packed_len, incoming);
    free(packed);
    return ok;
}
...
static int run_server(int listener_fd)
{
    ...
    pid_t child = -1;
    int pty_fd = -1, socket_fd = -1, result = 1;
    ...
    while (!server_stop) {
        struct pollfd ready[3] = {
            { .fd = listener_fd, .events = socket_fd < 0 ? POLLIN : 0 },
        ...
        if (terminal_changed && attached) {
            ...
            if (dirty != GHOSTTY_RENDER_STATE_DIRTY_FALSE
                ...
                if (!snapshot_send(socket_fd, &snapshot)) {
                    close(socket_fd);
                    socket_fd = -1;
                    attached = false;
                    sent_snapshot = false;
...
static int run_server_socket(const char *path)
{
    int listener = unix_socket(path, true);
    if (listener < 0) {
        perror("listen on Unix socket");
        return 1;
    }
    printf("ghostling server listening on %s\n", path);
...
static int run_client(int socket_fd)
{
    ...
    if (!input_send(socket_fd, &input_wire)) goto cleanup;
    ...
    while (!WindowShouldClose()) {
        ...
        if (input_wire.len && !input_send(socket_fd, &input_wire)) break;

        struct pollfd ready = { .fd = socket_fd, .events = POLLIN };
        while (poll(&ready, 1, 0) > 0 && (ready.revents & POLLIN)) {
            if (!snapshot_receive(socket_fd, &client_wire)) goto cleanup;
    ...
    input_send(socket_fd, &input_wire);
    result = 0;
...
int main(int argc, char **argv)
{
    if (argc == 3 && strcmp(argv[1], "--server") == 0)
        return run_server_socket(argv[2]);
    ...
    fprintf(stderr, "usage: %s --server SOCKET\n"
                    "       %s --connect SOCKET\n", argv[0], argv[0]);
    return 1;
}
